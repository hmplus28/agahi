...
<div class="cat-modal" data-cat-modal data-name="{{ $name ?? 'category_id' }}" data-selected="{{ $selectedCategoryId ?? '' }}">
    @unless($hideLabel)
        <label>دسته‌بندی <span class="req">*</span></label>
    @endunless
    <button type="button" class="cat-trigger" data-cat-open>
        <span data-cat-selected>{{ $selectedCat->title ?? 'انتخاب دسته‌بندی' }}</span>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 9l-7 7-7-7"/></svg>
    </button>
    <input type="hidden" name="{{ $name ?? 'category_id' }}" value="{{ $selectedCategoryId ?? '' }}" data-cat-input>
        ...
        <div class="cat-modal-box" role="dialog" aria-modal="true">
            ...
            <div class="cat-cols">
                <div class="cat-col" data-cat-group>
                    ...
                    <div class="cat-list">
                        @foreach($groups as $g)
                            <button type="button" class="cat-item{{ $preGroupId===$g->id?' active':'' }}" data-group="{{ $g->id }}" data-title="{{ $g->title }}">{{ $g->title }}</button>
                        @endforeach
                    </div>
                </div>
...
<script>
(function(){
    var tree = @json($treeJson).filter(c=>c.id);
    ...
    // initial selection
    @if($preGroupId)
    selected.group=@json((string)$preGroupId);
    @endif
    @if($preCollectionId)
    selected.collection=@json((string)$preCollectionId);
    @endif
    @if($preSubId)
    selected.sub=@json((string)$preSubId);
    @endif
