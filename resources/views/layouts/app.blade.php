...
<head>
    ...
    <script type="application/ld+json">
    {
        ...
        "url": "{{ url('/') }}",
        "description": "{{ config('agahi.site_description') }}",
        "inLanguage": "fa-IR",
        "potentialAction": {
            "@type": "SearchAction",
...
<body class="{{ request()->routeIs('admin.*') ? 'theme-admin' : '' }}">
    ...
    <div class="container header-top">
            ...
            @auth
                @if (auth()->user()->is_staff)
                    <nav class="role-switch" aria-label="تغییر نقش">
                        <a href="{{ route('user.dashboard') }}" class="{{ request()->routeIs('admin.*') ? '' : 'active' }}">حساب من</a>
                        <a href="{{ route('admin.dashboard') }}" class="{{ request()->routeIs('admin.*') ? 'active' : '' }}">پنل مدیریت</a>
                    </nav>
                @endif
            @endauth
        </div>
        ...
        @auth
            <div class="header-actions header-actions--end header-actions--guest">
                <a class="post-ad-button" href="{{ route('user.ads.create') }}"><span aria-hidden="true">＋</span> ثبت آگهی</a>
        ...
        @else
            <div class="header-actions header-actions--end header-actions--guest">
                <a class="post-ad-button" href="{{ route('guest.ad.create') }}"><span aria-hidden="true">＋</span> ثبت آگهی</a>
...
@include('public.partner_buttons')
...
<footer class="site-footer" role="contentinfo">
    <div class="container footer-inner">
        ...
        <nav aria-label="پیوندهای پایی">
            <a href="{{ route('search') }}">جست‌وجوی آگهی‌ها</a>
            <a href="{{ route('user.ads.create') }}">ثبت آگهی رایگان</a>
        ...
        <p class="copyright">© {{ jalali_year() }} {{ config('app.name') }} — تمام حقوق محفوظ است. | توسعه توسط <a href="https://daynacode.ir" target="_blank" rel="noopener" style="color:#5eead4;text-decoration:none;font-weight:600;">تیم دایناکد</a></p>
    </div>
</footer>
...
<div class="city-modal" id="city-modal" role="dialog" aria-modal="true" aria-label="انتخاب موقعیت" hidden
     ...
     data-selected-cities="{{ $selectedCityIds->implode(',') }}"
     data-selected-provinces="{{ $selectedProvinceIds->implode(',') }}">
    ...
    <div class="city-modal__panel">
        ...
        <div class="city-modal__quick">
            <label class="city-chip {{ $hasSelection ? '' : 'is-selected' }}">
                <input type="checkbox" data-city-all {{ $hasSelection ? '' : 'checked' }}>
                🇮🇷 کل ایران
            </label>
        </div>
        <form id="city-select-form" method="get" action="{{ route('search') }}">
            @foreach(request()->query() as $k => $v)
                @if(!in_array($k, ['cities', 'city', 'provinces', 'page', 'country']))
